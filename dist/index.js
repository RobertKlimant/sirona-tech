...
(() => {
  ...
  var set_size = () => {
    ...
    const on_resize = (e) => {
      const window_param = e.target;
      const window_height2 = window_param.innerHeight;
      const el2 = document.querySelector("[window-height]");
      el2.style.height = window_height2 + "px";
    };
    const window_height = window.innerHeight;
    const el = document.querySelector("[window-height]");
    el.style.height = window_height + "px";
    window.addEventListener("resize", on_resize);
  };

  // src/utils/scroll_trigger.ts
  var createScrollTrigger = (triggerElement, timeline2) => {
  ...
  // src/utils/other_pages.ts
  var title_other_animation = () => {
    const letters_fade_in = document.querySelectorAll("[letters-fade-in]");
    letters_fade_in.forEach((el) => {
